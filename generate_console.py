...
<body class="h-screen w-screen flex flex-col bg-slate-50 text-slate-800 font-sans select-none overflow-hidden text-xs">
  ...
  <div class="h-10 bg-white/95 backdrop-blur border-b border-slate-200 px-3 flex items-center justify-between shrink-0 z-20 shadow-xs overflow-x-auto whitespace-nowrap text-xs">
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <div class="flex items-center gap-1">
        ...
        <select id="sel-scenario" onchange="switchScenario(this.value)" class="bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 text-xs text-slate-800 font-medium focus:bg-white focus:border-blue-500">
          <option value="grid_9_square">九宫格库区</option>
          <option value="narrow_corridor">窄通道库区</option>
          <option value="open_yard">开阔堆场</option>
        </select>
      </div>
    ...
    <div class="flex items-center gap-1 shrink-0 mx-2">
      <span class="text-[11px] font-bold text-slate-500 mr-0.5">工位:</span>
      <button onclick="dispatchStation('S1')" class="px-1.5 py-0.5 rounded bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-medium text-[11px]">S1 拣选</button>
      <button onclick="dispatchStation('S2')" class="px-1.5 py-0.5 rounded bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-medium text-[11px]">S2 入库</button>
      <button onclick="dispatchStation('S3')" class="px-1.5 py-0.5 rounded bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-medium text-[11px]">S3 质检</button>
      <button onclick="dispatchStation('S4')" class="px-1.5 py-0.5 rounded bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-medium text-[11px]">S4 充电</button>
      <button onclick="dispatchStation('P0')" class="px-1.5 py-0.5 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 font-medium text-[11px]">P0 待命</button>
      <button onclick="cancelNavigation()" class="px-1.5 py-0.5 rounded bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-medium text-[11px] flex items-center gap-0.5">
  ...
  <script>
    ...
    // 航位推算平滑位姿 (Dead Reckoning Latency Compensation)
    const deadReckonPose = {
      x: 0,
      y: 0,
      yaw: 0,
      vx: 0,
    ...
    // --------------------------------------------------------------
    // 10Hz 实时遥测轮询与航位推算 (TELEMETRY LOOP)
    // --------------------------------------------------------------
    ...
    function startTelemetryLoop() {
      setInterval(async () => {
        ...
        try {
          const res = await fetch('/api/telemetry?full=1');
          if (res.ok) {
    ...
    function onTelemetryReceived(data) {
      ...
      // 同步推算基准
      deadReckonPose.x = data.x || 0;
      deadReckonPose.y = data.y || 0;
      deadReckonPose.yaw = data.yaw || 0;
      deadReckonPose.vx = data.vx || 0;
      deadReckonPose.vy = data.vy || 0;
      deadReckonPose.wz = data.wz || 0;
    ...
    function updateHUD(data) {
      ...
      const pts = (data.laser_scan && data.laser_scan.ranges) ? data.laser_scan.ranges.length : 720;
      if (hudLaser) hudLaser.textContent = `${pts} pts (10.0 Hz)`;
    ...
    async function dispatchStation(stationId) {
      const stationMap = {
        S1: { x: 3.0, y: 3.0, yaw: 0.0 },
        S2: { x: -3.0, y: 3.0, yaw: Math.PI / 2 },
        S3: { x: -3.0, y: -3.0, yaw: Math.PI },
        S4: { x: 3.0, y: -3.0, yaw: -Math.PI / 2 },
        P0: { x: 0.0, y: 0.0, yaw: 0.0 }
      };
      const goal = stationMap[stationId];
      if (!goal) return;
    ...
    function renderLoop2D() {
      if (cameraMode === '2d' && cv2D && ctx2D) {
        ...
        if (telemetry) {
          ...
          if (dt > 0 && dt < 0.25) {
            deadReckonPose.x = telemetry.x + telemetry.vx * Math.cos(telemetry.yaw) * dt;
            deadReckonPose.y = telemetry.y + telemetry.vx * Math.sin(telemetry.yaw) * dt;
            deadReckonPose.yaw = telemetry.yaw + telemetry.wz * dt;
          }
        }
        draw2DScene();
      }
    ...
    function renderLoop3D() {
      if (cameraMode !== '2d') {
        if (robot3DGroup) {
          robot3DGroup.position.set(deadReckonPose.x, deadReckonPose.y, 0);
          robot3DGroup.rotation.z = deadReckonPose.yaw;
        }

        // 跟随相机模式
        if (cameraMode === 'follow' && camera3D && controls3D) {
          controls3D.target.set(deadReckonPose.x, deadReckonPose.y, 0.5);
          camera3D.position.set(
            deadReckonPose.x - Math.cos(deadReckonPose.yaw) * 6,
            deadReckonPose.y - Math.sin(deadReckonPose.yaw) * 6,
            4
          );
        } else if (cameraMode === 'top' && camera3D && controls3D) {
          controls3D.target.set(deadReckonPose.x, deadReckonPose.y, 0);
          camera3D.position.set(deadReckonPose.x, deadReckonPose.y, 16);
        }

        // 更新 3D 点云
        if (telemetry && telemetry.laser_scan && telemetry.laser_scan.ranges && pointCloud3D) {
          ...
          ranges.forEach((r, idx) => {
            if (r > 0.05 && r < 12.0 && ptIdx < MAX_3D_PTS) {
              const a = deadReckonPose.yaw + angleMin + idx * angleInc;
              const px = deadReckonPose.x + Math.cos(a) * r;
              const py = deadReckonPose.y + Math.sin(a) * r;
              const pz = 0.3;
