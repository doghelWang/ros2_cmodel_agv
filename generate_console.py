...
<body class="h-screen w-screen flex flex-col bg-slate-50 text-slate-800 font-sans select-none overflow-hidden text-xs">
  ...
  <div class="flex-1 flex overflow-hidden relative">
    ...
    <div class="w-[380px] bg-white border-l border-slate-200 flex flex-col h-full shrink-0 shadow-sm overflow-hidden z-20">
      ...
      <div id="tab-panel-perturb" class="flex-1 p-3 overflow-y-auto space-y-3 hidden">
        <div class="pb-2 border-b border-amber-200">
          <div class="font-bold text-amber-900 text-xs flex items-center justify-between">
            ...
            <span id="perturb-pause-badge" class="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
              ▶ 运行中 (需先暂停)
            </span>
          </div>
          <div class="text-[11px] text-amber-700 mt-0.5">选取工业元素放入 PyBullet 仿真环境中，测试避障与重规划</div>
          <div id="perturb-pause-hint" class="mt-2 p-2 rounded bg-amber-50 border border-amber-200 text-[11px] text-amber-800 flex items-start gap-1.5 transition-all">
            <span class="text-base leading-none">⚠️</span>
            <span id="perturb-pause-hint-text">添加扰动模块前必须先<b>暂停物理仿真</b>。放置后无需弹窗确认，<b>恢复仿真</b>时自动生效进入物理碰撞引擎。</span>
          </div>
        </div>
        ...
        <!-- 确认注入按钮 -->
        <button id="btn-confirm-inject" onclick="confirmInjectElement()" class="w-full py-2 bg-slate-400 hover:bg-slate-500 text-white rounded-lg text-xs font-bold shadow-sm flex items-center justify-center gap-1.5 active:scale-95 transition-all">
          <i data-lucide="lock" class="w-4 h-4"></i> 将该元素放入仿真环境中 (需先暂停)
        </button>
  ...
  <script>
    ...
    function onTelemetryReceived(data) {
      ...
      // 同步暂停状态
      if (data.is_paused !== undefined && simPaused !== data.is_paused) {
        simPaused = data.is_paused;
      ...
      // 实时更新 Tab 5 扰动面板按钮状态与提示文案
      const pertBadge = document.getElementById('perturb-pause-badge');
      const pertHint = document.getElementById('perturb-pause-hint');
      const pertHintText = document.getElementById('perturb-pause-hint-text');
      const pertBtn = document.getElementById('btn-confirm-inject');
      if (pertBadge && pertHint && pertHintText && pertBtn) {
        if (data.is_paused) {
          pertBadge.textContent = '⏸ 已暂停 (可放置模块)';
          pertBadge.className = 'px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-300';
          pertHint.className = 'mt-2 p-2 rounded bg-blue-50 border border-blue-200 text-[11px] text-blue-800 flex items-start gap-1.5 transition-all';
          pertHintText.innerHTML = '仿真已暂停。选取元素并点击放置，<b>无需弹窗确认</b>；恢复仿真时物理实体立即生效！';
          pertBtn.className = 'w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold shadow-sm flex items-center justify-center gap-1.5 active:scale-95 transition-all';
          pertBtn.innerHTML = '<i data-lucide="plus-circle" class="w-4 h-4"></i> 将该元素放入仿真环境中 (待恢复生效)';
        } else {
          pertBadge.textContent = '▶ 运行中 (需先暂停)';
          pertBadge.className = 'px-2 py-0.5 rounded text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200';
          pertHint.className = 'mt-2 p-2 rounded bg-amber-50 border border-amber-200 text-[11px] text-amber-800 flex items-start gap-1.5 transition-all';
          pertHintText.innerHTML = '添加扰动模块前必须先<b>暂停物理仿真</b>。放置后无需弹窗确认，<b>恢复仿真</b>时自动生效进入物理碰撞引擎。';
          pertBtn.className = 'w-full py-2 bg-slate-400 hover:bg-slate-500 text-white rounded-lg text-xs font-bold shadow-sm flex items-center justify-center gap-1.5 active:scale-95 transition-all';
          pertBtn.innerHTML = '<i data-lucide="lock" class="w-4 h-4"></i> 将该元素放入仿真环境中 (需先暂停)';
        }
        if (window.lucide) lucide.createIcons();
      }
      ...
      if (navBadge) {
        if (data.is_paused) {
          navBadge.textContent = '⏸ 物理仿真暂停 (PAUSED)';
          navBadge.className = 'px-2 py-0.5 rounded text-[11px] font-semibold bg-amber-50 text-amber-700 border border-amber-200';
        } else if (data.nav_status === 'OBSTACLE_WAIT') {
          navBadge.textContent = '🛑 障碍边缘等待 (OBSTACLE_WAIT)';
          navBadge.className = 'px-2 py-0.5 rounded text-[11px] font-bold bg-rose-50 text-rose-700 border border-rose-200 animate-pulse';
        } else if (data.nav_status === 'NAVIGATING') {
          navBadge.textContent = '● 导航中 (NAVIGATING)';
          navBadge.className = 'px-2 py-0.5 rounded text-[11px] font-semibold bg-blue-50 text-blue-700 border border-blue-200 animate-pulse';
        } else {
          navBadge.textContent = '● 空闲就绪 (IDLE)';
          navBadge.className = 'px-2 py-0.5 rounded text-[11px] font-medium bg-slate-100 text-slate-600 border border-slate-200';
        }
      }
    ...
    function updateHUD(data) {
      ...
      if (hudObstacle) {
        if (minDist <= 1.36) {
          hudObstacle.textContent = `${minDist.toFixed(2)} m (已达障碍边缘·安全停车)`;
          hudObstacle.className = 'font-mono-num font-bold text-rose-600';
        } else if (minDist <= 2.15) {
          hudObstacle.textContent = `${minDist.toFixed(2)} m (减速区·跟进中)`;
          hudObstacle.className = 'font-mono-num font-bold text-amber-600';
        } else {
          hudObstacle.textContent = minDist < 11.0 ? `${minDist.toFixed(2)} m (安全巡航)` : '> 2.5 m (安全巡航)';
      ...
      if (hudStatus) {
        if (data.is_paused) {
          hudStatus.textContent = '⏸ 物理暂停';
          hudStatus.className = 'px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200';
        } else if (data.nav_status === 'OBSTACLE_WAIT') {
          hudStatus.textContent = '🛑 障碍边缘等待';
          hudStatus.className = 'px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200';
        } else if (data.nav_status === 'NAVIGATING') {
          hudStatus.textContent = '● 导航中';
          hudStatus.className = 'px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200';
        } else {
          hudStatus.textContent = '● 空闲就绪';
          hudStatus.className = 'px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200';
        }
      }
    ...
    async function confirmInjectElement() {
      ...
      if (!telemetry.is_paused) {
        showToast('⚠️ 仅在物理仿真暂停状态下允许添加扰动模块，请先点击顶栏暂停仿真！', 'warning');
        return;
      }
      ...
      try {
        ...
        if (res.ok) {
          ...
          // 放置物体后无需弹窗确定，仅展示轻量 toast 提示已布置，恢复仿真时生效
          showToast(`已布置 ${name} (X:${targetX.toFixed(2)}, Y:${targetY.toFixed(2)})，恢复仿真时正式生效！`, 'success');
        } else {
          showToast(resData.message || '添加失败，请先暂停物理仿真', 'warning');
        }
      } catch (e) {
        showToast('注入请求异常: ' + e, 'warning');
      }
    }
    ...
    function renderInjectedObstaclesList() {
      ...
      container.innerHTML = injectedObstacles.map((obs, idx) => {
        ...
        const isLive = obs.active !== false && (!telemetry || !telemetry.is_paused);
        const statusBadge = isLive 
          ? '<span class="text-[10px] px-1.5 py-0.5 rounded border font-medium bg-emerald-100 text-emerald-800 border-emerald-200">物理生效</span>'
          : '<span class="text-[10px] px-1.5 py-0.5 rounded border font-medium bg-amber-100 text-amber-800 border-amber-200">待生效 (暂停中)</span>';
        return `
    ...
    function update3DObstacles() {
      ...
      const curSig = JSON.stringify(curList.map(o => ({ id: o.id, x: o.x, y: o.y, w: o.w, h: o.h, type: o.type, active: o.active })));
